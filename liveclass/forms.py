from django import forms
from django.apps import apps
from .models import LiveClass


class LiveClassForm(forms.ModelForm):

    # 👇 Manually declare teacher field (lazy loading safe)
    teacher = forms.ModelChoiceField(
        queryset=apps.get_model("accounts", "Teacher").objects.none(),
        required=False
    )

    class Meta:
        model = LiveClass
        fields = [
            "subject",
            "class_room",
            "teacher",
            "title",
            "description",
            "start_time",
            "end_time",
        ]
        widgets = {
            "start_time": forms.DateTimeInput(attrs={"type": "datetime-local"}),
            "end_time": forms.DateTimeInput(attrs={"type": "datetime-local"}),
        }

    def __init__(self, *args, **kwargs):
        school = kwargs.pop("school", None)
        user = kwargs.pop("user", None)

        super().__init__(*args, **kwargs)

        # Lazy-load models safely
        Subject = apps.get_model("results", "Subject")
        SchoolClass = apps.get_model("students", "SchoolClass")
        Teacher = apps.get_model("accounts", "Teacher")

        if school:
            self.fields["subject"].queryset = Subject.objects.filter(school=school)
            self.fields["class_room"].queryset = SchoolClass.objects.filter(school=school)
            self.fields["teacher"].queryset = Teacher.objects.filter(school=school)

        # If teacher is creating → hide teacher dropdown
        if user and user.is_teacher_user:
            self.fields.pop("teacher")

        # Optional Tailwind styling
        for field in self.fields.values():
            field.widget.attrs.update({
                "class": "w-full border rounded-lg p-2 focus:ring-2 focus:ring-blue-400"
            })
